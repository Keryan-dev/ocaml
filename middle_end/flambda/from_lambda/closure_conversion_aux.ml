...
module Acc = struct
  ...
  let add_continuation_to_free_names ~cont ~has_traps ?args_approx t =
    Printf.eprintf "add cont %s%d\n" (Continuation.name cont) (Continuation.name_stamp cont);
  ...
  let remove_continuation_from_free_names cont t =
    Printf.eprintf "remove cont %s%d\n" (Continuation.name cont) (Continuation.name_stamp cont);
    { t with
      free_names =
...
module Let_cont_with_acc = struct
  let create_non_recursive acc cont handler
    ...
    | Zero when not (Continuation_handler.is_exn_handler handler) ->
      Printf.eprintf "removed let_cont %s%d\n" (Continuation.name cont) (Continuation.name_stamp cont);
      acc, body
    | _ ->
      acc,
